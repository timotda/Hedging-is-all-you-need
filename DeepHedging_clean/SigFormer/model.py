...
class Block(eqx.Module):
    ...
    def __call__(
        ...
        *,
        ...
        """Apply attention + MLP block with residuals.

        Args:
            x: List of signature tensors.
            key: Optional PRNG key split for sublayers.

        Returns:
            list[Array]: Updated tensors per signature level.
        """
...
class SigFormer(eqx.Module):
    ...
    def __call__(
        ...
        """Run SigFormer end-to-end to produce hedge actions.

        Args:
            x: Input price/feature path (seq_len, in_dim).
            key: PRNG key for stochastic layers.

        Returns:
            Array: Output sequence shaped (seq_len, out_dim).
        """
